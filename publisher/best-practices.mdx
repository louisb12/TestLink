...
{/*
  ...
  ═══════════════════════════════════════════════════════════════════════════
  DO NOT PUT COMMENTS INSIDE THE `export const` BLOCKS BELOW.

  Any JS or JSX comment inside an export block breaks Mintlify's PRODUCTION
  MDX compiler — the whole page is replaced with "A parsing error occured".
  `mint dev` and `mint validate` both accept them, so this is invisible
  locally; it only shows up on a real build. Verified against `mint export`:
  line comments, block comments, JSX comments inside returned markup, comments
  inside a data array, and trailing comments on a code line ALL fail.
  See decisions.md D-031.

  NOTE: this comment block must never contain the two-character sequence that
  closes an MDX comment. It terminates the block early — everything after it
  spills into the page as content and the build fails the same way. That is
  why the forms above are described in words rather than shown as syntax.

  Page-level MDX comments like this one — outside every export — are fine.
  `scripts/verify.sh` fails the build if a comment appears inside an export.

  So the notes that used to live inline are here instead:

  · Marker — the connector runs OUTWARD from the point (flipped when x < 50)
    so the note never covers the element it describes.

  · Marker, touch branch — floating chips push off a 390px screen (4 of 7 did)
    and hover has nowhere to go, so without hover the image gets numbered pins
    and the notes are listed under it, keyed by the same number. Detected with
    matchMedia hover/pointer, not a width breakpoint, because a narrow window
    on a laptop should still get chips.

  · Media, dialog — a native <dialog> opened with showModal() renders in the
    browser's TOP LAYER, above every stacking context. Necessary because
    Mintlify's #content carries Tailwind's `@container`
    (container-type: inline-size), which implies `contain: layout` and creates
    a stacking context: a plain fixed overlay at z-index 200 was still painted
    under the z-30 navbar. Also brings Escape-to-close and a focus trap.
    showModal() does not stop the page scrolling behind it in every browser,
    hence the explicit body scroll lock.

  · Media, `sized` — the image is sized explicitly so the marker layer maps to
    exactly the image box (hotspot coordinates are percentages OF THE IMAGE),
    and so a raster is never upscaled past what the display can resolve.

  · PromptCard, doCopy — three tiers, because the async Clipboard API is
    unavailable in plenty of real contexts (non-secure origins, embedded
    webviews, permission denied): navigator.clipboard, then an execCommand
    fallback, then reveal the prompt for manual selection.

  · Progress — the `--bp-pct` style prop is a genuinely dynamic value set after
    hydration by user interaction, which is the one case where the style prop
    is safe: there is no server-rendered value for it to mismatch.

  · Checklist useEffect — auto-advances as soon as the current stage is fully
    ticked.
  ═══════════════════════════════════════════════════════════════════════════
*/}
...
export const HINT_KEY = "almedia-link-lightbox-hint-seen-v2";

export const Media = ({ src, caption, ratio = "phone", kind = "image", emphasize = false, showCaption = false, overlay = null, hint = false, crop = null }) => {
  ...
  const hintKey = HINT_KEY + ":" + String(src).split("/").pop();
  ...
  const sized = (() => {
    ...
    const availW = vp.w * (narrow ? 0.86 : 0.5);
    const availH = vp.h * (narrow ? 0.62 : 0.74);
  ...
  const frameClass = crop
    ? "bp-frame bp-crop bp-crop-" + crop
    : "bp-frame bp-frame-" + ratio + (emphasize ? " bp-frame-emphasize" : "");
  ...
  return (
    <figure className="bp-media">
      <button type="button"
              ...
              onClick={() => { if (showHint) dismissHint(); setOpen(true); setActive(null); }}
              aria-label={spots.length
                ? "Open annotated view: " + caption + " (" + spots.length + " notes)"
                : "Enlarge: " + caption}>
        {crop ? (
          <span className="bp-crop-inner">
        ...
        {showHint ? (
          <span className="bp-hint" aria-hidden="true">
            <span className="bp-hint-halo" />
            <span className="bp-hint-scrim" />
            <span className="bp-hint-tap">
...
{/* ── Pointer arrow ────────────────────────────────────────────────────────
    A red arrow drawn on a screenshot. `left`/`top` are the position of the
    arrow TIP as a percentage of the image; `angle` is the direction it points
    (0 = right, 90 = down, 180 = left). Ported from the source guide, which used
    these to make clear which element in a busy game screen to look at.
    ─────────────────────────────────────────────────────────────────────────── */}

export const PointerArrow = ({ left, top, angle = 0, length = 40, label }) => (
  <span
    className="bp-arrow"
    aria-hidden="true"
    style={{
  ...
  >
    <svg viewBox="0 0 110 26" focusable="false">
      <g fill="none" stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 13 H 104" />
        <path d="M 90 4 L 105 13 L 90 22" />
      </g>
    </svg>
...
export const Panel = ({ variant, points, media, ratio, kind, hint = false }) => {
  ...
  return (
    <div className={"bp-panel" + (isDo ? " bp-panel-do" : "")}>
      ...
      {list.length > 1 ? (
        ...
        <Media src={list[0].src} caption={list[0].caption} ratio={ratio} kind={kind} hint={hint} />
      )}
...
export const DoDont = ({ doPoints, dontPoints, doMedia, dontMedia, ratio = "phone", kind = "image", hint = false }) => (
  <div className="bp-dodont">
    <Panel variant="do" points={doPoints} media={doMedia} ratio={ratio} kind={kind} hint={hint} />
    <Panel variant="dont" points={dontPoints} media={dontMedia} ratio={ratio} kind={kind} hint={hint} />
  </div>
);
...
export const copyToClipboard = (text, done, fail) => {
  const legacy = () => {
    try {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).then(done, () => { if (!legacy()) fail(); });
  } else if (!legacy()) {
    fail();
  }
};

export const CopyButton = ({ label, prompt, variant = "primary" }) => {
  ...
  return (
    <button
      ...
      onClick={() => copyToClipboard(prompt, done, done)}
    >
      {copied ? "Copied to your clipboard" : label}
    </button>
  );
};
...
export const PromptCard = ({ tag, title, copy, buttonLabel, prompt }) => {
  ...
  const doCopy = () => {
    const done = () => { setCopied(true); setTimeout(() => setCopied(false), 2400); };
    copyToClipboard(prompt, done, () => setShown(true));
  };
...
export const CORE_STAGES = [
  { group: "Pop-up placement", items: [
    ...
    "Reshow every few levels, max every 30 minutes",
...
3. If the player dismisses it, what brings it back? (Every few levels? Every X minutes of play time? Only once per session? Once per install?)
...
- Reshown every few levels, and at most once every 30 minutes of play time, to any player who has not linked yet.
- Rolled out to at least 90% of users.
...
<section className="bp-hero">
  ...
  <div className="bp-hero-inner">
    ...
    <div className="bp-hero-actions">
      <CopyButton label="Copy AI prompt: audit my pop-up" prompt={POPUP_PROMPT} />
      <CopyButton label="Copy AI prompt: audit my entry point" prompt={ENTRY_PROMPT} variant="secondary" />
    </div>
  </div>
</section>
...
<ol className="bp-placements">

  <li className="bp-place">
    <Media src="/images/best-practices/placement-homescreen.webp" crop="top"
      overlay={<PointerArrow left={31} top={25.5} angle={180} length={50}
    ...
    <div className="bp-place-body">
      <span className="bp-place-head">
        <span className="bp-place-n" aria-hidden="true">1</span>
        <span className="bp-num">Homescreen · start here</span>
      </span>
      <h4>Homescreen corner</h4>
      <p>
        Every unlinked player passes it every session, and every linked player uses it to reach
        Rewarded Progression or the Earn Tab.
      </p>
    </div>
  </li>

  <li className="bp-place">
    <Media src="/images/best-practices/placement-banner-rounded.png" crop="bottom"
      overlay={<PointerArrow left={64} top={91} angle={90} length={13}
    ...
    <div className="bp-place-body">
      <span className="bp-place-head">
        <span className="bp-place-n" aria-hidden="true">2</span>
  ...
  <li className="bp-place bp-place-wide">
    <Media src="/images/best-practices/placement-shop.png" ratio="shop"
      overlay={<PointerArrow left={82} top={32} angle={90} length={15}
    ...
    <div className="bp-place-body">
      <span className="bp-place-head">
        <span className="bp-place-n" aria-hidden="true">3</span>
